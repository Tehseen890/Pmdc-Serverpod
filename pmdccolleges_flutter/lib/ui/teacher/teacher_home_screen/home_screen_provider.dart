...
import 'package:pmdccolleges_flutter/core/services/database_storage_services.dart';
...
class TeacherHomeScreenProvider extends BaseViewModal {
    ...
    try {
      ...
      await client.videolecture.addVideoLecture(videoLecture).then((value) {
        if (value == true) {
          ...
          Navigator.of(context).pushReplacement(MaterialPageRoute(
              builder: (context) => TeacherVideoLectureScreen(index: index)));
        } else {
          ScaffoldMessenger.of(context)
              .showSnackBar(SnackBar(content: Text("Error, Try again")));
        }
    ...
    try {
      ...
      await client.assignment.addAssignment(assignment).then((value) {
        if (value == true) {
          ...
          Navigator.of(context).pushReplacement(MaterialPageRoute(
              builder: (context) => TeacherSubjectAssignments(index: index)));
        } else {
          ScaffoldMessenger.of(context)
              .showSnackBar(SnackBar(content: Text("Error, Try again")));
        }
  ...
  Future<void> deleteVideoLecture(noteId) async {
    setState(ViewState.busy);
    try {
      await client.videolecture.deleteVideoLectures(noteId).then((value) =>
          listOfVideoLecture.removeWhere((note) => note.id == noteId));
    } catch (e) {}
    setState(ViewState.idle);
  }
